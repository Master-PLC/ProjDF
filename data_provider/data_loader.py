...
class Dataset_PEMS(Dataset):
    ...
    def __read_data__(self):
        ...
        train_ratio = 0.6
        valid_ratio = 0.2
        border1s = [0, int(train_ratio * len(data)), int((train_ratio + valid_ratio) * len(data))]
        border2s = [int(train_ratio * len(data)), int((train_ratio + valid_ratio) * len(data)), len(data)]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
        ...
        data = df.fillna(method='ffill', limit=len(df)).fillna(method='bfill', limit=len(df)).values
...
class Dataset_M5(Dataset):
    def __init__(
        self, root_path, flag='train', size=None, features='MS', data_path='m5.csv',
        target='WI_3', scale=True, timeenc=0, freq='d', seasonal_patterns=None, 
        add_noise=False, noise_amp=0.1, noise_freq_percentage=0.05, noise_seed=2023, 
        noise_type='sin', data_percentage=1., cycle=7, time_feature_types=None, **kwargs
    ):
        # size [seq_len, label_len, pred_len]
        # info
        self.seq_len = size[0]
    ...
    def __read_data__(self):
        ...
        border1s = [0, 1913 - 28 - self.seq_len]
        border2s = [1913 - 28, 1913 + 1]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
...
class Dataset_M5_Fourier(Dataset_M5):
    def __init__(
        self, root_path, flag='train', size=None, features='MS', data_path='m5.csv',
        target='WI_3', scale=True, timeenc=0, freq='d', seasonal_patterns=None, 
        add_noise=False, noise_amp=0.1, noise_freq_percentage=0.05, noise_seed=2023, 
        noise_type='sin', data_percentage=1., num_freqs=16, cycle=7, time_feature_types=None, **kwargs
    ):
        super().__init__(
            root_path, flag, size, features, data_path, target, scale, timeenc, freq, seasonal_patterns, 
            add_noise, noise_amp, noise_freq_percentage, noise_seed, noise_type, data_percentage, cycle, time_feature_types, **kwargs
...
class Dataset_M5_PCA(Dataset_M5):
    def __init__(
        self, root_path, flag='train', size=None, features='MS', data_path='m5.csv',
        target='WI_3', scale=True, timeenc=0, freq='d', seasonal_patterns=None, 
        add_noise=False, noise_amp=0.1, noise_freq_percentage=0.05, noise_seed=2023, 
        noise_type='sin', data_percentage=1., trend_k=0.02, rank_ratio=1.0, 
        pca_dim="all", reinit=0, speedup_sklearn=0, cycle=7, time_feature_types=None, **kwargs
    ):
        super().__init__(
            root_path, flag, size, features, data_path, target, scale, timeenc, freq, seasonal_patterns, 
            add_noise, noise_amp, noise_freq_percentage, noise_seed, noise_type, data_percentage, cycle, time_feature_types, **kwargs
