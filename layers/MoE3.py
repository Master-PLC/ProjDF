...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, output_size, num_experts, hidden_size, noisy_gating=True, k=4):
        ...
        # self.experts = nn.ModuleList([My_expert(input_size, output_size) for i in range(self.num_experts)])
        self.experts = nn.ModuleList([nn.Linear(input_size, output_size) for i in range(self.num_experts)])
...
class My_expert(nn.Module):
    def __init__(self, input_dim, d_model, reduction=4):
        super(My_expert, self).__init__()
