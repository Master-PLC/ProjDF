...
class Exp_Long_Term_Forecast_Vali(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion, return_preds=False):
        ...
        if not return_preds:
            ...
            return total_loss, mae_loss, ot_dist_exact.item(), inputs, preds, trues
    ...
    def train(self, setting):
        ...
        train_fixed_data, train_fixed_loader = self._get_data(flag='train', shuffle=False)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            train_mse, train_mae, train_ot, epoch_inputs, epoch_preds, epoch_trues = self.vali(train_fixed_data, train_fixed_loader, criterion, return_preds=True)
            np.savez(
                os.path.join(res_path, f'epoch_data_{epoch}.npz'), # 注意后缀改成 .npz
                inputs=epoch_inputs.cpu().numpy(),
                preds=epoch_preds.cpu().numpy(),
                trues=epoch_trues.cpu().numpy()
            )
