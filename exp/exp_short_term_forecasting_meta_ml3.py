    ...
    def vali(self, train_loader, vali_loader, criterion):
        ...
        with torch.no_grad():
            ...
            if self.args.loss.lower() == 'smape':
                total_loss = criterion(x.detach(), self.args.frequency_map, pred, true, batch_y_mark)
            else:
                total_loss = criterion(pred, true)  # 标准损失
            total_cov_loss = self.A.get_loss(pred, true)  # 学习到的损失

        print('Validation cost time: {}'.format(time.time() - eval_time))
        self.model.train()
        self.A.train()
        return total_loss.item(), total_cov_loss.item()
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle) in enumerate(train_loader):
                ...
                if self.args.auxi_lambda:
                    loss_auxi = self.A.get_loss(outputs, batch_y)
